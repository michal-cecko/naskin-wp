...
{
    $employeeID = $_POST['employeeID'] ?? false;
    if (!$employeeID) {
        wp_send_json([
            'message' => "ID Zamestnanca nebolo nastavené."
        ], 403);
    }
    ...
    update_appointment_fields($appointment, $employeeID, $postID, $type);
...
function edit_appointment()
{
    ...
    update_appointment_fields($appointment, $employeeID, $id, $type);
...
function update_appointment_fields($data, $employee, $post_id, $type)
{
    ...
    // Must-update fields: Type and employee
    update_field('appointment_type', $type, $post_id);
    update_field('appointment_employee', $employee, $post_id);
    ...
    if ($type == "appointment") {
        ...
        if (!isset($customerID)) {
            $args = [
                'post_status' => 'publish',
                'post_type' => 'customer',
                'post_title' => $data['customer']['name'],
            ];
            $customerID = wp_insert_post($args);
            update_field('cust_name', $data['customer']['name'], $customerID);
            update_field('cust_email', $data['customer']['email'], $customerID);
            if (!empty($data['customer']['phone']))
                update_field('cust_phone', $data['customer']['phone'], $customerID);
        }

        $lastAppointment = get_field('cust_last-appointment', $customerID);
        if(strtotime($lastAppointment) < $datetimeFrom->getTimestamp()) {
            update_field('cust_last-appointment', $datetimeFrom->format("Y-m-d H:i"), $customerID);
        }
...
function get_appointments()
{
    ...
    if (empty($employeeID) || empty($timestamp) || empty($dateRange)) {
        wp_send_json([
    ...
    while ($appointments->have_posts()):
        ...
        $employeeID = get_field("appointment_employee");

        if (!isset($fetchedEmployees[$employeeID])) {
            $employee = get_user_by("ID", $employeeID);
            $fetchedEmployees[$employee->ID] = $employee;
        } else {
            $employee = $fetchedEmployees[$employeeID];
        }
    ...
    if ($q->have_posts()) {
        while ($q->have_posts()) {
            ...
            $finalPosts[] = [
                'id' => get_the_ID(),
                'name' => get_the_title(),
                'email' => get_field("cust_email"),
                'phone' => get_field("cust_phone"),
            ];
        }
    }
